...
class PdoMySQL{
	 ...
	 */
	public static function find($tables,$where=null,$fields='*',$group=null,$having=null,$record=null,$limit=null){
...
//		echo $sql;
	 ...
	 * @return Ambigous <boolean, unknown, number>
	 */
	public static function update($data,$table,$where=null,$record=null,$limit=0){
		foreach($data as $key=>$val){
			$sets.=$key."='".$val."',";
		}
		$sets=rtrim($sets,',');
		$sql="UPDATE {$table} SET {$sets} ".self::parseWhere($where).self::parserecord($record).self::parseLimit($limit);
		echo  $sql;
		return self::execute($sql);
	}
	/**
	 ...
	 * @return Ambigous <boolean, unknown, number>
	 */
	public static function delete($table,$where=null,$record=null,$limit=0){
		$sql="DELETE FROM `{$table}` ".self::parseWhere($where).self::parserecord($record).self::parseLimit($limit);
		return self::execute($sql);
	}
	/**
	 * 得到最后执行的SQL语句
	...
	public static function parserecord($record){
		$recordStr='';
		if(is_array($record)){
			$recordStr.=' record BY '.join(',',$record);
		}elseif(is_string($record)&&!empty($record)){
			$recordStr.=' record BY '.$record;
		}
		return $recordStr;
	}
	/**
