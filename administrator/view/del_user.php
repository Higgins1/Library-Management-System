                        ...
                        <!-- PAGE CONTENT BEGINS -->
  						<?php
								...
								$link=mysqli_connect("localhost","root","root","library");
	  							$query = mysqli_query($link,"select * from user where iduser='$ad_id';");
								$row = mysqli_num_rows($query);
	  							$query1 = mysqli_query($link,"select * from user where iduser='$ad_id' and 
									password='$psd';");
								$row1 = mysqli_num_rows($query1);
								if ($row1>0)
								{	
	  								$query = mysqli_query($link,"delete from user where iduser='$ad_id';");
									echo"删除用户成功！";
								}
								else {
									if ($row>0) echo"您输入的用户编号和密码不匹配。";
									else echo"您所要删除的用户不存在！";
								}
