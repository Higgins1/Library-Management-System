                        ...
                        <!-- PAGE CONTENT BEGINS -->
  						<?php
								...
								$link=mysqli_connect("localhost","root","root","library");
	  							$query = mysqli_query($link,"select * from book where idbook='$ad_id';");
								$row = mysqli_num_rows($query);
								if ($row>0)
								{	
	  								$query = mysqli_query($link,"select * from book where idbook='$ad_id' and state='0';");
									$row = mysqli_num_rows($query);									if ($row>0) echo"该图书不在馆内，请等待用户将图书归还后进行删除操作~";									else {
										$query = mysqli_query($link,"delete from book where idbook='$ad_id';");										echo"删除图书成功！";									}								}
								else {
									echo"您所要删除的图书不存在！";
								}
								
								echo'<form action="addbook.php">';
