                        ...
                        <!-- PAGE CONTENT BEGINS -->
  						<?php
								...
								$link=mysqli_connect("localhost","root","root","library");
	  							$query = mysqli_query($link,"select * from administrator where idadministrator='$ad_id';");								$row = mysqli_num_rows($query);
	  							$query1 = mysqli_query($link,"select * from administrator where idadministrator='$ad_id' and 									password='$psd';");
								$row1 = mysqli_num_rows($query1);
								if ($row1>0)
								{	
	  								$query = mysqli_query($link,"delete from administrator where idadministrator='$ad_id';");
									echo"删除管理员成功！";
								}
								else {									if ($row>0) echo"您输入的管理员编号和密码不匹配。";									else echo"您所要删除的管理员不存在！";								}
								
								echo'<form action="add_ad.php">';
