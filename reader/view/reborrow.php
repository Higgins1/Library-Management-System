                ...
                <div class="row">
                    <div class="col-xs-12">
						...
						<?php
								...
								$query=mysqli_query($link,"select * from user_borrow where iduser='$stu_id' and idbook='$book_id';");
        														   //从用户已经借阅的书籍中查找这本书								$rows=mysqli_num_rows($query);
								$query1=mysqli_query($link,"select * from user_borrow where iduser='$stu_id' and idbook='$book_id'																and renew_number<3;");															       //从用户已经借阅的书籍中查找没有达到续借上限的书
        						$rows1=mysqli_num_rows($query1);
								if ($rows1>0){
	  								$query = mysqli_query($link,"update user_borrow set renew_number=renew_number+1 											where iduser='$stu_id' and idbook='$book_id';");
	  								$query = mysqli_query($link,"update user_borrow set due_back=date(date_add(due_back,INTERVAL 1 month))
											where iduser='$stu_id' and idbook='$book_id';");
	  								$query = mysqli_query($link,"update borrowed_record set due_back=date(date_add(due_back,INTERVAL 1 month))
											where iduser='$stu_id' and idbook='$book_id';");
									if ($query)
									{	
										echo"续借成功！";
