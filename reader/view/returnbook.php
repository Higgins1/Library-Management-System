                ...
                <div class="row">
                    <div class="col-xs-12">
						...
						<?php
								...
								$query=mysqli_query($link,"select * from user_borrow where idbook='$book_id' and iduser='$stu_id';");										//在用户借阅的书单中找到对应的元组
        						$rows=mysqli_num_rows($query);
								if ($rows>0){
	  								$query = mysqli_query($link,"delete from user_borrow where idbook='$book_id' and iduser='$stu_id';");												//从用户借阅的书单中删除该书籍
									if ($query)
									{	
										$query = mysqli_query($link,"update book set state=1 where idbook='$book_id'");													//更新书籍在馆状态
										$query = mysqli_query($link,"update user set borrowed_book_num=borrowed_book_num-1 where iduser='$stu_id'");													//更新借书数量
										$result = mysqli_query($link,"select * from user_borrow where idbook='$book_id' and iduser='$stu_id';");										$row = mysqli_fetch_array($result,MYSQLI_NUM); 										$date_back = $row[3];										$query = mysqli_query($link,"update user set fine_amount=fine_amount+DATEDIFF($date_back,date(now())) div 10;");
										$query = mysqli_query($link,"update borrowed_record set return_date=date(now()) where idbook='$book_id' 
													and iduser='$stu_id' and return_date is null;");													//更新归还日期
										if ($query) echo"归还成功！";
									}
									else echo"归还失败!";
								}
